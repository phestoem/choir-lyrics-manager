...
/**
 * Handle AJAX search requests for suggestions

public function handle_ajax_search() {
    // Verify nonce
    if (!wp_verify_nonce($_POST['nonce'], 'clm_search_nonce')) {
...
public function handle_ajax_filter() {
	...
	// Verify nonce
    $nonce_valid = wp_verify_nonce($_POST['nonce'], 'clm_filter_nonce');
    ...
    // Verify nonce
    if (!wp_verify_nonce($_POST['nonce'], 'clm_filter_nonce')) {
        wp_die('Security check failed');
    }
    ...
    if (!empty($filters['orderby'])) {
        ...
        // Handle custom ordering for composer
	...
	// Add alphabet filter (starts_with)
    if (!empty($filters['starts_with'])) {
        $letter = strtoupper(sanitize_text_field($filters['starts_with']));
        
        // Use a custom query to filter by first letter of title
    ...
    if (!empty($filters['year_from']) || !empty($filters['year_to'])) {
        ...
        if (!empty($filters['year_from']) && !empty($filters['year_to'])) {
            $year_query['value'] = array(
                intval($filters['year_from']),
                intval($filters['year_to'])
            );
            $year_query['compare'] = 'BETWEEN';
        } elseif (!empty($filters['year_from'])) {
            $year_query['value'] = intval($filters['year_from']);
            $year_query['compare'] = '>=';
        } else {
            $year_query['value'] = intval($filters['year_to']);
            $year_query['compare'] = '<=';
        }
    ...
    if ($query->max_num_pages > 1) {
        ...
        <div class="clm-page-jump">
            ...
            <input type="number" id="clm-page-jump-input" min="1" max="<?php echo $query->max_num_pages; ?>" value="<?php echo $page; ?>">
...
*/
	...
	/**
 * Handle AJAX requests for shortcode filtering
...
public function handle_shortcode_filter() {
    ...
    // Genre filter
    if (!empty($_POST['filter_genre'])) {
        $tax_query[] = array(
            'taxonomy' => 'clm_genre',
            'field' => 'slug',
            'terms' => sanitize_text_field($_POST['filter_genre']),
        );
    }
    ...
    if (!empty($_POST['filter_language'])) {
        $tax_query[] = array(
            'taxonomy' => 'clm_language',
            'field' => 'slug',
            'terms' => sanitize_text_field($_POST['filter_language']),
        );
    }
    ...
    if (!empty($_POST['filter_difficulty'])) {
        $tax_query[] = array(
            'taxonomy' => 'clm_difficulty',
            'field' => 'slug',
            'terms' => sanitize_text_field($_POST['filter_difficulty']),
        );
    }
    ...
    if (!empty($_POST['filter_order'])) {
        $args['order'] = sanitize_text_field($_POST['filter_order']);
    }
